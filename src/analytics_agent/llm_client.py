...
class OpenRouterClient:
    ...
    def complete_json(self, messages: list[dict[str, str]], max_tokens: int = 3000) -> tuple[str, Any]:
        ...
        for model in self.config.models:
            for json_mode in (True, False):
                ...
                for attempt in range(1, self.config.max_model_attempts + 1):
                    try:
                        logger.info("OpenRouter request: model=%s mode=%s attempt=%s", model, mode_label, attempt)
                        content = self._complete(model, messages, max_tokens=max_tokens, json_mode=json_mode)
                        logger.info("OpenRouter response received: model=%s mode=%s attempt=%s", model, mode_label, attempt)
                        ...
                        if self._is_auth_error(error):
                            raise RuntimeError(f"OpenRouter authentication failed: {error}") from error
                        if not self._is_retryable_error(error) and not isinstance(error, ValueError):
                            logger.warning("Model %s failed with non-retryable error: %s; trying next model.", model, error)
                            break
                        logger.warning("Model %s failed in %s attempt %s: %s", model, mode_label, attempt, error)
                logger.info("Model %s exhausted %s; trying next mode/model.", model, mode_label)

        if last_error is not None:
            details = " | ".join(failures[-8:])
            raise RuntimeError(f"All configured OpenRouter models failed. Recent failures: {details}") from last_error
    ...
    def _is_retryable_error(self, error: Exception) -> bool:
        ...
        retryable_markers = ("timeout", "timed out", "rate", "temporarily", "overloaded", "connection", "json")
    ...
    def _to_langchain_messages(self, messages: list[dict[str, str]]) -> list[tuple[str, str]]:
        role_map = {"assistant": "ai", "user": "human", "system": "system"}
        return [(role_map.get(message["role"], "human"), message["content"]) for message in messages]
