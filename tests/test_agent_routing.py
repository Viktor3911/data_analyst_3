...
def _step(tmp_path: Path, *, stderr: str = "", artifacts: list[Path] | None = None) -> AgentStep:
    return AgentStep(
        number=1,
        model="test-model",
        reason="проверка",
    ...
    route = agent._route_after_llm(
        {
            ...
            "steps": [_step(tmp_path, artifacts=[artifact])],
            "repair_attempts": 0,
        }
    )
    ...
    route = agent._route_after_llm(
        {
            ...
            "steps": [_step(tmp_path, stderr="Traceback")],
            "repair_attempts": 0,
        }
    )
